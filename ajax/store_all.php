...
if (isset($_POST['stockOpening']) && isset($_POST['catName1']) && isset($_POST['fdate']) && isset($_POST['tdate'])) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $totalPurchasAmount = floatval($row['totalPurchasAmount']);
        $totalPurchaseQty = floatval($row['totalPurchaseQty']);
        ...
        if ($totalPurchasAmount == 0) {
            $less = 0;
        } else {

            $less = $totalPurchasAmount / ($totalPurchaseQty - 1);
        }

        // $oneLess= $totalPurchasAmount/$less;

        if ($totalPurchaseQty != 0) {
            $cloaQtyAmt = $less;
            //($totalPurchasAmount+$oneLess)/$totalPurchaseQty;
